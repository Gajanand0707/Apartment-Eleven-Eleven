    ...
    <Link href={blog.readMoreUrl || "#"}>
      ...
      >
        ...
        <div className="w-full md:w-3/5 p-6 md:p-12 flex flex-col justify-center md:justify-between">
          <h3 className="text-2xl md:text-3xl lg:text-4xl font-goudy-old font-bold text-black mb-3">{blog.title}</h3>
          {blog.subtitle && (
            <p className="text-xl md:text-2xl font-goudy-old text-black font-bold mb-3 line-clamp-2">
              {blog.subtitle}
            </p>
          )}
          <div>
          <p className=" text-[20px] md:text-2xl font-goudy leading-relaxed mb-4 line-clamp-3">{blog.description}</p>
          <span
            className="block text-blue-500 font-goudy text-xl md:text-2xl hover:underline text-right ml-auto pointer-events-none"
          >
            Read more...
          </span>
          </div>
